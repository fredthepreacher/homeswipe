...
const VIEW_AS_ROLES = [
  { role: "consumer" as const, label: "Renter / Buyer", desc: "Swipe cards, search & saved homes", Icon: Home, home: "/", gradient: "from-sky-900/60 to-sky-800/30", ring: "ring-sky-600/40 hover:ring-sky-500/70", badge: "bg-sky-500/20 text-sky-300" },
  { role: "broker" as const, label: "Broker", desc: "Listings dashboard & inquiries", Icon: Briefcase, home: "/broker", gradient: "from-emerald-900/60 to-emerald-800/30", ring: "ring-emerald-600/40 hover:ring-emerald-500/70", badge: "bg-emerald-500/20 text-emerald-300" },
  { role: "landlord" as const, label: "Landlord", desc: "Property management & inquiries", Icon: Building2, home: "/broker", gradient: "from-amber-900/60 to-amber-800/30", ring: "ring-amber-600/40 hover:ring-amber-500/70", badge: "bg-amber-500/20 text-amber-300" },
];
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white pb-24 overflow-y-auto">
      <header className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-gray-900 px-5 pt-8 pb-5">
        ...
        <p className="text-indigo-300/70 text-xs">Welcome back, {user?.firstName ?? user?.fullName?.split(" ")[0] ?? "Admin"}</p>
      </header>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {VIEW_AS_ROLES.map((r) => (
              <button
                key={r.role}
                onClick={() => { setPreviewRole(r.role); router.push(r.home); }}
            ...
            {(["all", "consumer", "broker"] as SideFilter[]).map((s) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold transition ${
                  side === s ? "bg-indigo-500 text-white" : "bg-gray-800 text-gray-400 hover:text-gray-200"
                }`}
              >
            ...
            {filtered.map((log) => (
              <div
                ...
                className={`flex items-start gap-3 p-3 rounded-xl transition-all ${
                  newIds.has(log.id) ? "bg-indigo-900/50 ring-1 ring-indigo-500/50" : "bg-gray-800/50"
                }`}
              >
