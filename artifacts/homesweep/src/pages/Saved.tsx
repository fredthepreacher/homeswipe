...
import { useGetSavedListings, type Listing } from "@workspace/api-client-react";
import { Heart, MapPin, Bed, Bath, Square, MessageCircle, Inbox } from "lucide-react";
import { formatPrice } from "@/lib/utils";
import { Link, useLocation } from "wouter";
import { motion } from "framer-motion";
import { ConsumerInbox } from "@/pages/ConsumerInbox";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {savedListings.map((property, idx) => (
            <SavedPropertyCard
              ...
              property={property}
              index={idx}
              onMessage={() => navigate(`/messages/new/${property.id}`)}
            />
          ))}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {/* Message button — only available for matched (saved) homes */}
        <button
          onClick={onMessage}
