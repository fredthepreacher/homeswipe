...
import { ArrowLeft, CheckCircle2, AlertCircle, DollarSign, MapPin, BedDouble, Bath, Square, Image, FileText, Tag, Play } from "lucide-react";
import { brokerApi } from "@/lib/broker-api";

const BROKER_AD_AFTER = 2; // show ad after every Nth listing upload
...
export default function BrokerAddListing() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Track broker listing count for ad gate
      ...
      if (next % BROKER_AD_AFTER === 0) {
        ...
        setSuccess(true);
        setTimeout(() => navigate("/broker/listings"), 1500);
      }
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background px-6 relative">
                ...
                <div className="p-5 flex flex-col items-center gap-3">
                  ...
                  {adCountdown === 0 && (
                    <motion.button
                      ...
                      onClick={() => { setShowAd(false); navigate("/broker/listings"); }}
        ...
        >
          ...
          {!showAd && (
            <button onClick={() => navigate("/broker/listings")} className="text-primary font-semibold text-sm">
              View My Listings →
            </button>
          )}
        </motion.div>
    ...
    <div className="min-h-screen flex flex-col bg-background pb-24 overflow-y-auto">
      <header className="px-5 pt-7 pb-4 sticky top-0 bg-background/95 backdrop-blur-xl z-10 border-b border-border/40">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/broker/listings")} className="text-muted-foreground hover:text-foreground transition">
