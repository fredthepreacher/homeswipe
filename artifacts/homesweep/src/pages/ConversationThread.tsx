...
export default function ConversationThread({ conversationId, newListingId }: Props) {
  const { user } = useAuth();
  const [, navigate] = useLocation();

  const [convId, setConvId]           = useState<number | null>(conversationId ? parseInt(conversationId) : null);
  ...
  /* ── Init: create or load conversation ── */
  useEffect(() => {
    async function init() {
      try {
        if (newListingId) {
          const listingIdNum = parseInt(newListingId);
          const conv = await messagesApi.getOrCreate(listingIdNum);
          setConvId(conv.id);
        }
      } catch (err: any) {
